...
class TableRenderer(OutputRenderer):
    ...
    def __init__(
        ...
        self._query_columns = self.validate_formatters(valid_titles)
    ...
    def correct_columns(self) -> None:
        ...
        result: List[List] = [
            self.derived[c] if c in self.derived else [c] for c in self._query_columns
        ]
        # flatten
        ...
        # remove duplicates
        self._query_columns = list(sorted(set(flat_result)))
