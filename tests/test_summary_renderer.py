...
@pytest.fixture
def some_jobs():
    """A few test jobs for generating a summary output."""
    jobs = []
...
def test_summary_only_formatter(some_jobs):
    """Reportseff output only summary format."""
    renderer = summary_renderer.SummaryRenderer()
    result = renderer.format_jobs(some_jobs)
    assert result == "There were 7 jobs"
